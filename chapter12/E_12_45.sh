#!/bin/bash
   ...
   # 这事实上一个"Monte Carlo" 蒙特卡洛模拟的非常简单的实例:

   #+ 由于要使用随机抽样统计来估算数学函数, 所以使用伪随机数来模拟真正的随机.
   #+ 蒙特卡洛模拟是一种由现实事件抽象出来的数学模型,
   #  想象有一个完美的正方形土地, 边长为 0000 个单位.
   #  在这块土地的中间有一个完美的圆形湖,
   #+ 这个湖的直径是 10000 个单位.
   #  这块土地的绝大多数面积都是水, 当然只有 4 个角上有一些土地.
   #  (可以把这个湖想象成为使这个正方形的内接圆.)
   #
   #  我们将使用老式的大炮和铁炮弹
   #+ 向这块正方形的土地上开炮.
   #  所有的炮弹都会击中这块正方形土地的某个地方.
   #+ 或者是打到湖上, 或者是打到  个角的土地上.
   #  因为这个湖占据了这个区域大部分地方,
 ...
 #
   ...
   # 然而, 对于 shell 脚本来说一定会作些让步的,
   #+ 因为它肯定不能和那些内建就支持浮点运算的编译语言相比.5 #
   # 当然就会降低精度.
   ...
   get_random()
   {
     ...
     SEED=$(head -1 /dev/urandom | od -N1  | awk '{print $2}')
...
EOF
   ...
   do
      xCoord=$(get_random)   # 取得随机的x 与 y 的坐标
      ...
      printf "  Xc = %d   "$xCoord
      printf "  Yc = %d   "$yCoord
      printf "  Distance = %d   "$distance  # 到湖中心的距离
