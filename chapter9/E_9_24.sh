...
#  $RONDOM 在每次调用的时候，返回一个不同的随机值

#  指定范围是 :0-32767(有符号的16-bit整数)
    ...
    # 如果你需要在一个特定范围产生一个随机int ,那么使用"modulo"（模)
    # 操作，这将返回一个除法操作的馀数
    ...
    #+ 那么建立一个test 循环，来丢弃所有产生对比这个数小的随机数
	...
	let "number %= $RANGE"   # 让$number 依比例落在$RANGE 范围内.
    ...
    #+ (右移 14 位将把所有为全部清空,除了第 15 位,因为有符号,所以第 16 位是符号位).
    ...
    # 是否让 SPOTS=7 比加 1 更好呢?解释行或者不行的原因?
