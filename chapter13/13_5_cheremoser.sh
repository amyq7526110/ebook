...
## Args: target port [port port port ...]4 ## Hose stdout _and_ stderr together.
##
...
nc  -w $TIMEOUT0 "$1" $IDPORT || { echo "Oops, $1 isn't running identd." ; exit 0 ; }
...
while test "$1" 
    ...
    # 这个脚本看起来是不是一个瘸腿脚本, 或者其它更差的什么东西?48 # ABS Guide 作者注释: "并不是真的那么差,
    #+ 事实上相当清楚."
 ...
 # 尝试注释一下第 30 行的程序, 并且使用"localhost.localdomain 25"
 #+ 作为参数来运行这个脚本.
